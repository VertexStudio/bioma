...
def rerank(query, texts):
    # Prepare the inputs
    inputs = tokenizer([query]*len(texts), texts, return_tensors='pt', padding=True, truncation=True)
    ...
    ranked_texts = [{'index': idx, 'score': score.item()} for idx, score in enumerate(scores)]
    return ranked_texts
