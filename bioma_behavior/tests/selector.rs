...
use bioma_behavior::composites::Selector;
use bioma_behavior::prelude::*;
use tokio::sync::mpsc;

#[tokio::test]
async fn test_behavior_selector_1st_succeeds() {
    let (telemetry_tx, mut telemetry_rx) = mpsc::channel::<BehaviorTelemetry>(1000);

    let selector_0 = BehaviorId::new("selector-0");
    ...
    bt.add_node(&selector_0, Selector::new(children)).await;
    ...
    let expected_telemetry = r#"
        [bt-0] bioma::core::Selector(selector-0): Ok(Shutdown) - InitBegin
        [bt-0] bioma::core::Selector(selector-0): Ok(Initialized) - InitEnd
        [bt-0] bioma::core::Selector(selector-0): Ok(Initialized) - TickBegin
        ...
        [bt-0] bioma::core::Selector(selector-0): Ok(Success) - TickEnd
        [bt-0] bioma::core::Selector(selector-0): Ok(Success) - ShutdownBegin
        [bt-0] bioma::core::Mock(mock-0): Ok(Success) - ShutdownBegin
        [bt-0] bioma::core::Mock(mock-0): Ok(Shutdown) - ShutdownEnd
        [bt-0] bioma::core::Selector(selector-0): Ok(Shutdown) - ShutdownEnd
    "#
    .trim();
...
#[tokio::test]
async fn test_behavior_selector_2nd_succeeds() {
    let (telemetry_tx, mut telemetry_rx) = mpsc::channel::<BehaviorTelemetry>(1000);
...
#[tokio::test]
async fn test_behavior_selector_none_succeeds() {
    let (telemetry_tx, mut telemetry_rx) = mpsc::channel::<BehaviorTelemetry>(1000);
    ...
    let expected_telemetry = r#"
        ...
        [bt-0] bioma::core::Selector(selector-0): Ok(Failure) - TickEnd
        [bt-0] bioma::core::Selector(selector-0): Ok(Failure) - ShutdownBegin
