...
use bioma_behavior::composites::All;
...
#[tokio::test]
async fn test_behavior_all_succeeds() {
...
#[tokio::test]
async fn test_behavior_all_1st_fails() {
    let (telemetry_tx, mut telemetry_rx) = mpsc::channel::<BehaviorTelemetry>(1000);
    ...
    let expected_telemetry = r#"
        ...
        [bt-0] bioma::core::All(all-0): Ok(Failure) - TickEnd
        [bt-0] bioma::core::All(all-0): Ok(Failure) - ShutdownBegin
        [bt-0] bioma::core::Delay(delay-1): Ok(Success) - ShutdownBegin
        [bt-0] bioma::core::Mock(mock-1): Ok(Success) - ShutdownBegin
        [bt-0] bioma::core::Mock(mock-1): Ok(Shutdown) - ShutdownEnd
        [bt-0] bioma::core::Delay(delay-1): Ok(Shutdown) - ShutdownEnd
...
#[tokio::test]
async fn test_behavior_all_1st_fails_interrupts() {
    let (telemetry_tx, mut telemetry_rx) = mpsc::channel::<BehaviorTelemetry>(1000);
...
#[tokio::test]
async fn test_behavior_all_2nd_fails() {
    let (telemetry_tx, mut telemetry_rx) = mpsc::channel::<BehaviorTelemetry>(1000);
...
#[tokio::test]
async fn test_behavior_all_2nd_fails_interrupts() {
    let (telemetry_tx, mut telemetry_rx) = mpsc::channel::<BehaviorTelemetry>(1000);
