...
import { useResizableElement } from "../../hooks/useResizableElement.ts";
...
const Sidebar: React.FC = () => {
  ...
  const [isVisible, setIsVisible] = useState<boolean>(sidebarVisible);
  ...
  const prevVisibleRef = useRef<boolean>(sidebarVisible);
  ...
  useEffect(() => {
    ...
    if (sidebarVisible !== prevVisibleRef.current) {
      ...
      console.log(
        `[Sidebar] Current width: ${currentWidthRef.current}px, Last saved width: ${lastWidthRef.current}px`
      );
